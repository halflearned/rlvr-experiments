...
def _replace_rmsnorm_with_hf_compatible(module: nn.Module) -> None:
    ...
    for name, child in list(module.named_children()):
        if isinstance(child, nn.RMSNorm):
            ...
            _replace_rmsnorm_with_hf_compatible(child)


def _apply_rotary_emb_hf_compatible(
...
def _patch_rope_for_hf_compatibility() -> None:
    ...
    qwen3_model.apply_rotary_emb = _apply_rotary_emb_hf_compatible
    ...
    Attention.forward.__globals__["apply_rotary_emb"] = _apply_rotary_emb_hf_compatible
    ...
    logger.info(f"Patch successful: {new_fn is _apply_rotary_emb_hf_compatible}")


def _convert_rope_cache_to_float32(module: nn.Module) -> None:
    ...
    for child in module.children():
        _convert_rope_cache_to_float32(child)


class TitanModel(torch.distributed.checkpoint.stateful.Stateful):

    @record
    def __init__(self, job_config: JobConfig, trainable=True, hf_compatible=False):
        ...
        #
        # Enable via hf_compatible=True parameter or RLVR_HF_COMPATIBLE=1 environment variable.
        if self.hf_compatible or os.environ.get("RLVR_HF_COMPATIBLE") == "1":
            logger.info("Enabling HuggingFace-compatible precision for numerical parity...")

            # 1. Replace RMSNorm with float32 variance computation
            for model_part in self.model_parts:
                _replace_rmsnorm_with_hf_compatible(model_part)

            # 2. Convert rope_cache to float32 (recompute from scratch)
            for model_part in self.model_parts:
                _convert_rope_cache_to_float32(model_part)

            # 3. Patch RoPE to use float32 computation
            _patch_rope_for_hf_compatibility()
