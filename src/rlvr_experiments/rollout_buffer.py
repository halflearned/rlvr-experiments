...
class RolloutBuffer(Generic[T]):
    ...
    async def put(self, item: T, version: int) -> None:
        ...
        print(f"[TEMPLOG buffer.put] Putting item version={version}, queue_size_before={self.size()}")
        entry = RolloutEntry(item=item, version=version, reads_remaining=self._max_reads)
        await self._queue.put_async(entry)
        self._stats["put"] += 1
        print(f"[TEMPLOG buffer.put] Put complete, queue_size_after={self.size()}, total_puts={self._stats['put']}")
    ...
    async def _pop_one(self, min_version: int | None = None) -> T:
        ...
        while True:
            loop_count += 1
            print(f"[TEMPLOG _pop_one] Waiting for item, loop={loop_count}, min_version={min_version}, queue_size={self.size()}")
            entry: RolloutEntry[T] = await self._queue.get_async()
            print(f"[TEMPLOG _pop_one] Got entry version={entry.version}, reads_remaining={entry.reads_remaining}")

            # Discard if stale
            if min_version is not None and entry.version < min_version:
                self._stats["evicted_stale"] += 1
                print(f"[TEMPLOG _pop_one] Evicted stale entry (version {entry.version} < {min_version})")
                continue
            ...
            if entry.reads_remaining > 0:
                await self._queue.put_async(entry)
                print(f"[TEMPLOG _pop_one] Re-queued entry, reads_remaining={entry.reads_remaining}")
            else:
                self._stats["evicted_exhausted"] += 1
                print(f"[TEMPLOG _pop_one] Entry exhausted, evicted")
    ...
    async def pop(
        ...
        """
        print(f"[TEMPLOG buffer.pop] Starting pop, batch_size={batch_size}, min_version={min_version}, queue_size={self.size()}")
        entries = []
        group_sizes = []
        ...
        for attempt in range(max_attempts):
            if len(entries) >= batch_size:
                print(f"[TEMPLOG buffer.pop] Got enough entries ({len(entries)}/{batch_size}), breaking")
                break

            print(f"[TEMPLOG buffer.pop] Attempt {attempt+1}/{max_attempts}, entries_so_far={len(entries)}, queue_size={self.size()}")
            ...
            print(f"[TEMPLOG buffer.pop] Got item, rewards shape={rewards.shape}, sum={reward_sum:.2f}, var={reward_var:.4f}")
            if torch.allclose(rewards, rewards[0]):
                self._stats["skipped_zero_var"] += 1
                print(f"[TEMPLOG buffer.pop] SKIPPED zero-variance entry (all rewards={rewards[0].item():.2f})")
                continue

            entries.append(item)
            group_sizes.append(rewards.shape[0])
            print(f"[TEMPLOG buffer.pop] ACCEPTED entry, now have {len(entries)}/{batch_size}")

        if not entries:
            print(f"[TEMPLOG buffer.pop] WARNING: No valid batches after {max_attempts} attempts. Stats: {self.get_stats()}")
            return None

        if len(entries) < batch_size:
            print(f"[TEMPLOG buffer.pop] Partial batch: got {len(entries)}/{batch_size} entries")
