...
class CheckpointManager:
    ...
    def dcp_save(
        ...
        """
        Instrumented version for timing.
        """
        print("hello from dcp_save")
        import time
        import torch
        ...
        if to_hf:
            ...
            t_hf_end = time.perf_counter()

            logger.warning(f"[TIMING] sd_adapter.to_hf: {t_hf_end - t_hf_start:.3f}s")

            # -------------------------
            # 2. Create HuggingFaceStorageWriter
            # -------------------------
            ...
            logger.warning(f"[TIMING] HFStorageWriter init: {t_writer_end - t_writer_start:.3f}s")
        ...
        # -------------------------
        # 3. DCP Save call (the heavy part)
        # -------------------------
        t_dcp_start = time.perf_counter()
        ...
        t_dcp_end = time.perf_counter()
        logger.warning(f"[TIMING] dcp.save / async_save: {t_dcp_end - t_dcp_start:.3f}s")


        # -------------------------
        # 4. Consolidation (if needed)
        # -------------------------
        if to_hf and self.sd_adapter.fqn_to_index_mapping:
            ...
            logger.warning(f"[TIMING] safetensors consolidation: {t_cons_end - t_cons_start:.3f}s")


        # -------------------------
        # 5. Garbage collection (optional)
        # -------------------------
        if enable_garbage_collection:
            t_gc_start = time.perf_counter()
            GarbageCollection.collect("GC collection invoked by checkpointer.")
            t_gc_end = time.perf_counter()
            logger.warning(f"[TIMING] GC: {t_gc_end - t_gc_start:.3f}s")
        ...
        logger.warning(f"[TIMING] TOTAL dcp_save({checkpoint_id}): {t1 - t0:.3f}s")
