...
def build_job_config() -> "JobConfig":
    ...
    return cfg_mgr.parse_args(["--job.config-file", "configs/training.toml"])


def init_distributed(job_config) -> ParallelDims:
    if "RANK" not in os.environ or "WORLD_SIZE" not in os.environ:
        raise RuntimeError(
            "Distributed init using env:// requires torchrun.\n"
            "Example:\n  torchrun --standalone --nnodes=1 --nproc_per_node=2 script.py"
        )
