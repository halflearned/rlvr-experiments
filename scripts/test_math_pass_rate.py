#!/usr/bin/env python3
"""
Test script to evaluate Qwen 1.7B Base on MATH dataset using vLLM.

Loads randomly selected prompts from MATH (levels 3-5), generates completions
...
Usage:
    python scripts/test_math_pass_rate.py [--num-prompts 100] [--n 512] [--seed 42]
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Test Qwen 1.7B Base on MATH")
    parser.add_argument(
        "--model-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of prompts to sample from MATH (default: 100)",
    )
    parser.add_argument(
        "--n",
        type=int,
    ...
    parser.add_argument(
        ...
        default="test",
        choices=["train", "test"],
        help="MATH split to use (default: test)",
    )
    parser.add_argument(
        "--levels",
        type=str,
        default="3,4,5",
        help="Comma-separated list of levels to include (default: 3,4,5)",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def load_and_sample_prompts(split: str, num_prompts: int, seed: int, levels: list[int]) -> list[dict]:
    """Load MATH and randomly sample prompts from specified levels."""
    print(f"Loading MATH {split} split (levels {levels})...")
    ...
    for subj in subjects:
        ...
        for i, row in enumerate(ds):
            if row['level'] in level_strs:
                ...
                if answer is None:
                    continue  # Skip if no boxed answer
    ...
    print(f"Total: {len(all_rows)} problems from levels {levels}")
...
def format_prompts(
    rows: list[dict],
    tokenizer,
    system_prompt: str = "Solve the following math problem step by step. Provide the final answer inside \\boxed{}",
...
def worker(gpu_id, prompts, model_path, sampling_params_dict, tp_size, result_queue):
    ...
    llm = LLM(
        ...
        max_model_len=4096,  # Longer for MATH
...
def main():
    ...
    rows = load_and_sample_prompts(args.split, args.num_prompts, args.seed, levels)
    ...
    for i, row in enumerate(rows):
        ...
        if args.verbose:
            print(f"\n[{i+1}/{len(rows)}] {prompt_id} ({result.level})")
    ...
    # By subject
    print("\nResults by subject:")
    for subject in sorted(set(r.subject for r in results)):
    ...
    # Compute level and subject breakdowns for saving
    level_breakdown = {}
    for level in sorted(set(r.level for r in results)):
        level_results = [r for r in results if r.level == level]
        level_correct = sum(r.num_correct for r in level_results)
        ...
        level_breakdown[level] = {
            "pass_rate": level_correct / level_total if level_total else 0,
            "pass_at_k": sum(1 for r in level_results if r.num_correct > 0) / len(level_results) if level_results else 0,
    ...
    for subject in sorted(set(r.subject for r in results)):
        ...
        subject_breakdown[subject] = {
            "pass_rate": subj_correct / subj_total if subj_total else 0,
    ...
    output_file = output_dir / f"math_L{levels_str}_{model_name}_{timestamp}.json"
