#!/bin/bash
# Run all pass@k evaluations with n=1024 for Qwen3-1.7B-Base on TRAINING sets
...
echo "=============================================="
echo "Starting all pass@k evaluations with n=1024"
...
echo ""
echo "[1/4] GSM8k evaluation (7473 train prompts × 1024 completions)"
...
echo ""
echo "[2/4] MATH evaluation (7500 train prompts × 1024 completions)"
echo "Start time: $(date)"
python3 scripts/eval_pass_rate.py \
    --dataset math \
    --split "$SPLIT" \
...
echo ""
echo "[3/4] MBPP evaluation (374 train prompts × 1024 completions)"
echo "Start time: $(date)"
python3 scripts/eval_pass_rate.py \
    --dataset mbpp \
    --split "$SPLIT" \
