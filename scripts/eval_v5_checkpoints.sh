#!/bin/bash
# Eval script for v5 checkpoints (beta=0.03, lr=2e-5)
# Run on primary node using GPU 6 (training uses 0-5)

set -e
cd /efs/rlvr-experiments

CHECKPOINT_DIR="/efs/rlvr-experiments/checkpoints"
RESULTS_DIR="/efs/rlvr-experiments/experiments/math_only_minerva_v5/evals"
mkdir -p "$RESULTS_DIR"

# Use GPU 6 only (GPU 7 may be used for v6 evals)
GPU=6
...
while true; do
    # Find v5 checkpoints that haven't been evaluated yet
    for ckpt in $(ls "$CHECKPOINT_DIR" 2>/dev/null | grep "math_only_minerva_v5_step" | sort -V); do
