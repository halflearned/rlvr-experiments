...
from rlvr_experiments.algorithms.grpo import RolloutSample, Batch, BatchStats, RewardStats, _bucket_size
...
class OPSDLoss(torch.nn.Module):
    """On-Policy Self-Distillation loss.

    Uses the Schulman k3 KL estimator (same as GRPO):
        r = log(teacher / student) = teacher_logprobs - trainer_logprobs
        KL ≈ exp(r) - r - 1

    This has a minimum at r=0 (student == teacher) and pushes the student
    distribution toward the teacher's.

    Matches GRPOLoss interface: forward() returns scalar tensor,
    get_debug_metrics() returns dict.
    """
    ...
    def forward(self, logits, completion_ids, teacher_logprobs, padding_mask, prompt_lens, temperature=1.0):
        trainer_logprobs, _ = compute_logprobs(
            logits,
            completion_ids,
            prompt_lens=prompt_lens,
            temperature=temperature,
        )
        teacher_lp = teacher_logprobs.to(trainer_logprobs.device, dtype=torch.float32)

        # Schulman k3 estimator: KL(πθ || q) ≈ exp(log(q/πθ)) - log(q/πθ) - 1
        log_ratio = teacher_lp - trainer_logprobs  # log(teacher / student)
        kl_t = torch.exp(log_ratio) - log_ratio - 1.0

        mask = padding_mask.to(kl_t.device, dtype=torch.float32)
        per_response_loss = (kl_t * mask).sum(dim=-1) / mask.sum(dim=-1).clamp_min(1.0)
        loss = per_response_loss.mean()
        self._last_debug = {
            "opsd_loss": float(loss.detach()),
            "mean_kl": float((kl_t.detach() * mask).sum() / mask.sum().clamp_min(1.0)),
            "mean_log_ratio": float((log_ratio.detach() * mask).sum() / mask.sum().clamp_min(1.0)),
            "mean_trainer_lp": float((trainer_logprobs.detach() * mask).sum() / mask.sum().clamp_min(1.0)),
            "mean_teacher_lp": float((teacher_lp.detach() * mask).sum() / mask.sum().clamp_min(1.0)),
        }
        return loss
...
class OPSDCompletion:
    """A single completion with its teacher logprobs. This is the atomic training unit."""
    ...
    teacher_logprobs: torch.Tensor # [completion_len]
    prompt_len: int
...
def make_opsd_batch(
    ...
    teacher_lps = torch.stack([pad1d(c.teacher_logprobs, padded_completion_len) for c in completions])
...
async def main() -> None:
    ...
    async def compute_teacher_logprobs_for_completion(
        teacher_template: str,
        completion_token_ids: list[int],
    ) -> torch.Tensor | None:
        """Compute teacher logprobs for a single completion.

        Args:
            teacher_template: The teacher prompt text (already built with hints/feedback).
            completion_token_ids: The student's completion tokens to score.

        Returns [1, actual_comp_len] tensor or None if teacher prompt doesn't fit.
        """
        ...
        input_ids = torch.tensor([teacher_seq], dtype=torch.long)
        comp_ids = torch.tensor([completion_token_ids], dtype=torch.long)
        prompt_lens = torch.tensor([teacher_prompt_len])

        with trace_span("teacher_logprobs"):
            teacher_lps = await rollout.compute_logprobs(
                input_ids, comp_ids, prompt_lens, temperature=policy_temperature
            )
        return teacher_lps
    ...
    async def produce_epoch():
        async def process_one(item):
            ...
            for idx in kept_indices:
                ...
                teacher_lps = await compute_teacher_logprobs_for_completion(
                    teacher_tmpl, comp_token_ids,
                )
                if teacher_lps is None:
                    continue
                ...
                opsd_completions.append(OPSDCompletion(
                    ...
                    teacher_logprobs=teacher_lps[0, :actual_len],
    ...
    for epoch in range(training.get("num_epochs") or 999999):
        ...
        async for mb_completions in minibatches(producer):
            ...
            print(f"[epoch {epoch}] step={trainer.version} loss={avg_loss:.4f} grad_norm={grad_norm:.4f} "
                  ...
                  f"mean_kl={dbg.get('mean_kl', 0):.4f} mean_lr={dbg.get('mean_log_ratio', 0):.4f} "
                  f"trainer_lp={dbg.get('mean_trainer_lp', 0):.4f} teacher_lp={dbg.get('mean_teacher_lp', 0):.4f}")
